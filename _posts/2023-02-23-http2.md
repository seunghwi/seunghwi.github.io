...
|이전 상태 |다음 상태|설명|
|-|-|---|
|IDLE|OPEN|클라이언트가 스트림을 생성하고 데이터를 보내기 위해 준비합니다.|
|OPEN|HALF_CLOSED_REMOTE|클라이언트가 요청 데이터를 모두 전송하고 서버에게 해당 스트림에서 더 이상데이터를 보내지 않겠다는 것을 알립니다.|
|OPEN|HALF_CLOSED_LOCAL|서버가 응답 데이터를 모두 전송하고 클라이언트에게 해당 스트림에서 더 이상 데이터를 보내지 않겠다는 것을 알립니다.|
|HALF_CLOSED_REMOTE|CLOSED|서버가 해당 스트림에서 더 이상 데이터를 보내지 않겠다는 것을 알립니다.|
|HALF_CLOSED_LOCAL|CLOSED|클라이언트가 해당 스트림에서 더 이상 데이터를 보내지 않겠다는 것을 알립니다.|
|OPEN|CLOSED|스트림이 닫힙니다. 데이터 전송이 완료되거나 오류가 발생했을 경우 발생합니다.|
